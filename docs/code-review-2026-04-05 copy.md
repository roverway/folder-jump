...
| # | 问题 | 位置 | 说明 |
|---|------|------|------|
| 1 | `WinWaitActive` 超时参数错误 | `path_switcher.ahk:233` | 2000 应为 2（秒 vs 毫秒） |

### P1（建议尽快修复）

| # | 问题 | 位置 | 说明 |
|---|------|------|------|
| 2 | TC 面板应使用 `wParam=9/10` 精确获取 left/right 语义 | `adapters/totalcmd.ahk` | 替代 `ControlGetPos` 坐标推断 |
| 3 | `InputHook.OnEnter` 仅触发一次 | `lib/selection_ui.ahk:181-183` | 首次 Enter 后 Hook 停止 |

### P2（建议后续迭代修复）

| # | 问题 | 位置 | 说明 |
|---|------|------|------|
| 4 | 控件安全访问函数重复 | `hotkey_manager.ahk` / `path_switcher.ahk` | 合并到 `utils.ahk` |
| 5 | 路径归一化函数重复 | `path_collector.ahk` / `path_switcher.ahk` / `dopus.ahk` | 合并到 `utils.ahk` |
| 6 | TC 适配器日志级别过高 | `adapters/totalcmd.ahk` | 常规轮询应用 `LogDebug` |
| 7 | `CleanupGui` 缺少重入保护 | `lib/selection_ui.ahk:276` | 多个关闭路径可能同时触发 |
| 8 | `TryNavigateFileDialogFast` 恢复文件名时未检查窗口存活 | `lib/path_switcher.ahk:92` | `ControlSetText` 恢复可能抛异常 |
| 9 | `FindDOpusRT` 硬编码安装路径 | `adapters/dopus.ahk:228-237` | 应增加注册表查询或配置项 |
| 10 | Explorer `for` 循环缺少外层 `try` | `adapters/explorer.ahk:19` | COM 枚举可能中途断开 |
| 11 | 方法一的 Sleep 延时使用固定值 | `lib/path_switcher.ahk:83,89` | 不同机器性能差异大 |
...
| 15 | `DOpusRT` 临时文件名每次使用 `A_TickCount` | `adapters/dopus.ahk:26` | 可改为固定名称覆盖写入 |
